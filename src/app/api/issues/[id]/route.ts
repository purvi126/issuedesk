...
export async function PATCH(
    ...
    try {
        ...
        const wasResolvedBefore = existingIssue.status === "RESOLVED";
        ...
        const isResolvedNow = updatedIssue.status === "RESOLVED";

        if (
            status === "RESOLVED" &&
            !wasResolvedBefore &&
            isResolvedNow &&
            typeof updatedIssue.createdByEmail === "string" &&
            updatedIssue.createdByEmail.trim()
        ) {
            try {
                await sendIssueResolvedEmail({
                    to: updatedIssue.createdByEmail.trim(),
                    issueTitle: updatedIssue.title?.trim() || "(Untitled)",
                    issueId: updatedIssue._id?.toString?.() ?? id,
                });
            } catch (emailError) {
                console.error("[issue PATCH by id] resolved email failed:", emailError);
            }
        }

        return NextResponse.json(
            { ok: true, issue: serializeIssue(updatedIssue) },
            { status: 200 }
        );
    } catch (err: unknown) {
        const message = err instanceof Error ? err.message : "Unknown error";
