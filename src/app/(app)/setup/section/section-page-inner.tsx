  ...
  return (
    <main className="min-h-[calc(100vh-64px)] px-4 py-10 sm:px-8">
      <div className="mx-auto max-w-4xl">
        <h1 className="text-3xl font-semibold tracking-tight text-slate-900 dark:text-white sm:text-4xl">Choose section</h1>
        <p className="mt-2 text-sm text-slate-600 dark:text-white/60">Tap one to continue.</p>
        ...
        <div className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-6 dark:border-white/10 dark:bg-white/[0.04]">
          ...
          {next ? (
            <div className="mt-4 text-xs text-slate-500 dark:text-white/45">
              After setup, you'll be redirected to: <span className="text-slate-700 dark:text-white/70">{next}</span>
            </div>
          ) : null}
        </div>
      </div>
    ...
    <button
      ...
      className={[
        ...
        active
          ...
          : "border-slate-200 bg-white hover:border-blue-300 hover:bg-blue-50 dark:border-white/10 dark:bg-white/5 dark:hover:border-blue-400/25 dark:hover:bg-blue-500/5",
      ].join(" ")}
    >
      <div className="relative mx-auto mb-3 h-40 w-40 overflow-hidden rounded-3xl border border-slate-200 bg-slate-50 dark:border-white/10 dark:bg-white/[0.03]">
        <Image src={img} alt={title} fill className="object-contain p-3" />
      </div>
      <div className="text-lg font-semibold text-slate-900 dark:text-white/90">{title}</div>
      <div className="mt-1 text-sm text-slate-600 dark:text-white/55">Tap to select</div>
    </button>
  );
}